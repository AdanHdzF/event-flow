...
public class NotificationSubscriber {
	...
	public NotificationSubscriber(EventBus eventBus, NotificationService notificationService) {
		...
		eventBus.subscribe("inventory.reserved", InventoryReservedData.class, this::onInventoryReserved,
				"notification");
		eventBus.subscribe("inventory.insufficient", InventoryInsufficientData.class, this::onInventoryInsufficient,
				"notification");
		...
		eventBus.subscribe("products.changed", ProductChangedData.class, this::onProductChanged, "notification");
	}
	...
	private void onInventoryInsufficient(InventoryInsufficientData event) {
		notificationService.notify("STOCK_INSUFICIENTE", event.orderId,
				"Producto: " + event.productId + " (solicitado: " + event.requestedQuantity + ", disponible: "
	...
	private void onProductChanged(ProductChangedData event) {
		notificationService.notify("PRODUCTO_ACTUALIZADO", event.productId,
				"Producto: " + event.name + " (Precio: $" + event.price + ", Stock: " + event.stock + ", Operación: "
						+ event.operation + ")");
	}
